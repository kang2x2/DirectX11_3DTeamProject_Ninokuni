...
void CStelliaState_Rage1Loop_Explosion::Enter_State(void* pArg)
{
	...
	// 디버깅으로 범위를 확인하기 위해 두 벡터를 선언 및 초기화.
...
void CStelliaState_Rage1Loop_Explosion::Tick_State(_float fTimeDelta)
{
	...
	// XMVector3AngleBetweenNormals : 두 벡터 사이의 각도를 라디안값으로 반환. 
	// 두 벡터가 정규화가 되어 있지 않아도 내부적으로 정규화한다.(라디안을 반환하기 때문)
